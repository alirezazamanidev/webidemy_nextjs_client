...
export async function GetAllCourses({ page = 1, pre_page = 8, sort = '', search = '' }: { page?: number, pre_page?: number, sort?: string, search?: string }) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_BASE_API_URL}/courses/filter?page=${page}&pre_page=${pre_page}&sort=${sort}&search=${search}`);
