...
import CardBlog from "./CardBlogLayout"
import SearchBarBlogayout from "./SearchBarBlogLayout"
const filterItem = [
...
export default function BlogsListLayout({ blogData }: props) {

  const [blogList, setBlogList] = useState<Course[]>(blogData.data ?? []);
  ...
  const getblogsListByChangeQuery = async () => {
    let coursesData = await GetAllBlogs({ page, pre_page: 8, sort, category });
    setBlogList(coursesData.data);
  }
  ...
  return (
    <>
      ...
      <div className=" border-solid border-b pb-16 border-gray-700 my-10 ">
...

        {
          blogList.length === 0 ? (
            <div className=" flex flex-col gap-5 justify-center items-center ">
              ...
              <h2 className=" text-3xl text-gray-500">مقاله ای یافت نشد!</h2>
            </div>
          ) : (
        ...
        <ReactCustomPaginate page={page} pageCount={blogData.pages} onPageChangeHandler={onPageChangeHandler} />
      </div >
